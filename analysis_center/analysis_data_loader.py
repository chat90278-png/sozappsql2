...
from .analysis_deadlines import build_deadline_rows
...
def _configure_readonly_connection(conn: sqlite3.Connection) -> sqlite3.Connection:
...
def _readonly_connect(path: Path) -> sqlite3.Connection:
    ...
    conn = _configure_readonly_connection(sqlite3.connect(uri, uri=True))
    try:
        # WAL modunda kalmış fakat -wal/-shm sidecar'ı bulunmayan arşiv/kopya
        # STS dosyalarında ilk schema sorgusu SQLite'ın recovery yazımı istemesine
        # neden olabilir. Önce normal read-only bağlantıyı deneriz; canlı STS'de
        # mevcut WAL görünürlüğü böylece korunur. Yalnızca readonly recovery hatası
        # oluşursa immutable snapshot fallback kullanılır.
    ...
    immutable_uri = f"file:{resolved}?mode=ro&immutable=1"
    fallback = _configure_readonly_connection(sqlite3.connect(immutable_uri, uri=True))
    fallback.execute("SELECT 1 FROM sqlite_master LIMIT 1").fetchone()
    return fallback
...
class AnalysisDataLoader:
    ...
    def _read_acceptances(self, conn: sqlite3.Connection, contracts: List[Dict[str, Any]], systems: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for raw_row in conn.execute("SELECT * FROM deliveries ORDER BY contract_id,sort_order,id").fetchall():
            ...
            out.append({"id": r.get("id"), "contract_id": r.get("contract_id"), "system_id": r.get("system_id"), "platform": contract.get("platform", ""), "contract_no": contract.get("contract_no", ""), "system_name": system.get("name") or _text(r.get("system_name")), "name": _text(r.get("name")), "status": _text(r.get("status")), "acceptance_date": _text(r.get("acceptance_date")), "planned_acceptance_date": _text(r.get("planned_acceptance_date") or payload.get("planned_acceptance_date")), "planned_delivery_date": _text(r.get("planned_delivery_date") or payload.get("planned_delivery_date")), "completion_date": _text(payload.get("completion_date")), "planned_total": total.get("planned_total", 0.0), "delivered_total": total.get("delivered_total", 0.0)})
        return out
