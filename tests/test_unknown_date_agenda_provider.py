...
def test_fully_unknown_contract():
    item = UnknownDateAgendaProvider().build(_context(), _bundle(_source()))[0]
    assert item.detail_payload["date_kind"] == "fully_unknown"


def test_month_unknown_day_system():
    item = UnknownDateAgendaProvider().build(_context(), _bundle(_source("system", "2026-07-TBD")))[0]
    assert item.description == "Yıl ve ay belli, gün TBD."


def test_year_only_delivery():
    item = UnknownDateAgendaProvider().build(_context(), _bundle(_source("delivery", "2026-TBD-TBD")))[0]
    assert "kabul tarihi" in item.title


def test_exact_date_is_skipped():
    assert UnknownDateAgendaProvider().build(_context(), _bundle(_source(raw="2026-07-20"))) == ()


def test_na_or_malformed_date_is_skipped():
    provider = UnknownDateAgendaProvider()
    assert provider.build(_context(), _bundle(_source(raw="-"))) == ()
    assert provider.build(_context(), _bundle(_source(raw="not-a-date"))) == ()


def test_completed_unknown_source_is_skipped():
    assert UnknownDateAgendaProvider().build(_context(), _bundle(_source(raw="TBD", status="Tamamlandı"))) == ()


def test_unknown_key_stable_when_raw_date_changes():
    provider = UnknownDateAgendaProvider()
    a = provider.build(_context(), _bundle(_source(raw="TBD")))[0]
    b = provider.build(_context(), _bundle(_source(raw="2026-TBD-TBD")))[0]
    assert a.key == b.key
