...
def test_current_v17_with_v16_shape_is_rejected_instead_of_silent_noop(
    tmp_path: Path,
):
    ...
    error = exc_info.value
    assert "şema sürümü ile gerçek veri yapısı uyuşmuyor" in error.user_message
    assert "schema_fingerprint_mismatch=v17" in error.technical_detail
...
def test_missing_future_fingerprint_contract_fails_closed(
    ...
    assert "schema_fingerprint_not_registered=v17" in exc_info.value.technical_detail
