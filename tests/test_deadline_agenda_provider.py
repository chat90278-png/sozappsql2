...
def test_contract_deadline_item():
    item = DeadlineAgendaProvider().build(_context(), [_source()])[0]
    assert item.contract_id == 1 and item.kind == "deadline"


def test_system_deadline_item():
    item = DeadlineAgendaProvider().build(_context(), [_source("system")])[0]
    assert item.system_id == 2 and "System A" in item.title


def test_delivery_uses_planned_acceptance_when_actual_missing():
    item = DeadlineAgendaProvider().build(_context(), [_source("delivery")])[0]
    assert item.detail_payload["date_field"] == "planned_acceptance_date"


def test_completed_source_is_skipped():
    assert DeadlineAgendaProvider().build(_context(), [_source(status="Tamamlandı")]) == ()


def test_non_exact_date_is_skipped():
    assert DeadlineAgendaProvider().build(_context(), [_source(raw="TBD")]) == ()


def test_more_than_sixty_days_is_skipped():
    assert DeadlineAgendaProvider().build(_context(), [_source(raw="2026-09-10")]) == ()


def test_overdue_priority_and_description():
    item = DeadlineAgendaProvider().build(_context(), [_source(raw="2026-07-10")])[0]
...
def test_deadline_key_is_stable_across_stage_change():
    ...
    one = provider.build(_context(), [_source(raw="2026-07-12")])[0]
    fifteen = provider.build(_context(), [_source(raw="2026-07-26")])[0]
...
def test_deadline_boundaries():
    ...
    for days, version in expected.items():
        ...
        assert provider.build(_context(), [_source(raw=raw)])[0].version == version
    raw_61 = date.fromordinal(_context().today.toordinal() + 61).isoformat()
    assert provider.build(_context(), [_source(raw=raw_61)]) == ()
