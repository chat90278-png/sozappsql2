...
def _print_sentinel(summary: dict[str, Any]) -> None:
    ...
    print("GUNDEMIM_DIFF_GATE_BEGIN")
    for key, value in pairs:
        print(f"{key}={value}")
    for index, detail in enumerate((summary.get("feature_only_failure_details") or [])[:MAX_SENTINEL_FAILURES], start=1):
...
def main() -> int:
    ...
    try:
        ...
        if not summary["requirements_match"]:
            ...
            summary["gate_reason"] = "Baseline and feature requirements.txt bytes differ; same dependency environment assumption is invalid."
        ...
        summary["feature_only_failure_details"] = [feature_detail_map[node] for node in feature_only_nodes]
        ...
        if failed_feature_prerequisites:
            summary["gate"] = "FAIL"
            summary["gate_reason"] = "Feature prerequisite command failed: " + ", ".join(failed_feature_prerequisites)
        elif feature_only_nodes:
            summary["gate"] = "FAIL"
            summary["gate_reason"] = f"Feature introduced {len(feature_only_nodes)} failing test node(s) relative to the exact original baseline."
        else:
            summary["gate"] = "PASS"
            summary["gate_reason"] = "Exact baseline and feature JUnit failure-node sets contain no feature-only failing test node; targeted feature prerequisites all passed."
    except Exception as exc:
        summary["gate"] = "INCOMPLETE"
        summary["gate_reason"] = _normalize_text(f"{type(exc).__name__}: {exc}")[:MAX_MESSAGE_CHARS]
