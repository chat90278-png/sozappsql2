...
def _parse_junit(path: Path) -> dict[str, Any]:
    ...
    if tag == "testsuite" or (tag == "testsuites" and "tests" in root.attrib):
        totals = {
...
def _write_summary(summary: dict[str, Any]) -> None:
    ...
    (OUTPUT_ROOT / "stage2a-summary.json").write_text(
...
def _print_sentinel(summary: dict[str, Any]) -> None:
    ...
    print("GUNDEMIM_STAGE2A_GATE_BEGIN")
    ...
    print("GUNDEMIM_STAGE2A_GATE_END")
...
def main() -> int:
    ...
    summary: dict[str, Any] = {
        ...
        "gate": "INCOMPLETE",
        "gate_reason": "Stage 2A differential validation did not complete.",
    }
    ...
    try:
        ...
        targeted_path = OUTPUT_ROOT / "stage2a-targeted.txt"
        ...
        if failed_prerequisites:
            summary["gate"] = "FAIL"
            summary["gate_reason"] = (
                "Feature prerequisite command failed: " + ", ".join(failed_prerequisites)
            )
            ...
            summary["gate"] = "PASS"
            summary["gate_reason"] = (
                "Exact accepted Stage 1 and Stage 2A JUnit failure-node sets contain no "
                "feature-only failing node; all targeted prerequisites passed."
            )
    except Exception as exc:
        summary["gate"] = "INCOMPLETE"
        summary["gate_reason"] = f"{exc.__class__.__name__}: {_normalize_text(exc)[:MAX_MESSAGE_CHARS]}"
