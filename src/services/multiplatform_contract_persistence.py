...
def install_multiplatform_contract_persistence_fix() -> None:
    ...
    are platform-scoped. The legacy writer chose the first linked platform as its
    write target. This wrapper keeps the linked-platform order/primary platform
    intact, but temporarily puts ``ci.platform_id`` first for the platform-scoped
    system and delivery upsert performed by the existing writer.
    """
    ...
    def write_contract_for_active_platform(
        ...
        if existing_contract_id and not int(getattr(ci, "entry_start_row", 0) or 0):
