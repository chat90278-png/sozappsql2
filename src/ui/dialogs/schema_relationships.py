...
def group_relationships_by_source(relationships: Iterable[dict]) -> list[tuple[str, list[dict]]]:
    groups: dict[str, list[dict]] = {}
    for relationship in relationships:
        groups.setdefault(str(relationship["source_table"]), []).append(relationship)
    return [
        (source_table, sorted(items, key=relationship_key))
        for source_table, items in sorted(groups.items())
    ]


def filter_relationship_groups(relationships: Iterable[dict], query: str = "") -> list[tuple[str, list[dict]]]:
    normalized = str(query or "").strip().casefold()
    groups = group_relationships_by_source(relationships)
    if not normalized:
        return groups
    filtered: list[tuple[str, list[dict]]] = []
    for source_table, items in groups:
        if normalized in source_table.casefold():
            filtered.append((source_table, items))
            continue
        matches = [
            relationship
            for relationship in items
            if normalized in " ".join(
                str(relationship[key]).casefold()
                for key in ("source_table", "source_column", "target_table", "target_column")
            )
        ]
        if matches:
            filtered.append((source_table, matches))
    return filtered
